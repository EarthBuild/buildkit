	...
	if rhs := m.Entries; rhs != nil {
		...
		for k, v := range rhs {
			if vtpb, ok := interface{}(v).(interface{ CloneVT() *types.Stat }); ok {
				...
				tmpContainer[k] = gogoproto.Clone(v).(*types.Stat)
			}
		}
...
func (m *StatFileResponse) CloneVT() *StatFileResponse {
	...
	if rhs := m.Stat; rhs != nil {
		if vtpb, ok := interface{}(rhs).(interface{ CloneVT() *types.Stat }); ok {
			...
			r.Stat = gogoproto.Clone(rhs).(*types.Stat)
		}
	}
	if len(m.unknownFields) > 0 {
		r.unknownFields = make([]byte, len(m.unknownFields))
...
func (this *StatFileResponse) EqualVT(that *StatFileResponse) bool {
	...
	} else if !gogoproto.Equal(this.Stat, that.Stat) {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}
	...
	if len(m.Entries) > 0 {
		for iNdEx := len(m.Entries) - 1; iNdEx >= 0; iNdEx-- {
			if vtmsg, ok := interface{}(m.Entries[iNdEx]).(interface {
				...
				encoded, err := gogoproto.Marshal(m.Entries[iNdEx])
				if err != nil {
					return 0, err
				}
				i -= len(encoded)
...
func (m *StatFileResponse) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.Stat != nil {
		if vtmsg, ok := interface{}(m.Stat).(interface {
			...
			encoded, err := gogoproto.Marshal(m.Stat)
			if err != nil {
				return 0, err
			}
			i -= len(encoded)
	...
	for iNdEx < l {
		...
		case 1:
			...
			if unmarshal, ok := interface{}(m.Entries[len(m.Entries)-1]).(interface {
				...
				if err := gogoproto.Unmarshal(dAtA[iNdEx:postIndex], m.Entries[len(m.Entries)-1]); err != nil {
					return err
				}
			}
			iNdEx = postIndex
	...
	for iNdEx < l {
		...
		case 1:
			...
			if unmarshal, ok := interface{}(m.Stat).(interface {
				...
				if err := gogoproto.Unmarshal(dAtA[iNdEx:postIndex], m.Stat); err != nil {
					return err
				}
			}
			iNdEx = postIndex
